...
const HeroSection = ({ slice }: HeroSectionProps): JSX.Element => {
  return (
    ...
    >
      <div className={`w-5/12 min-h-[${slice.primary.heroimage.dimensions?.height}px] flex flex-col justify-center gap-y-4 bg-light text-secondary pl-10 p-24 max-lg:w-full`}>
      ...
      <div className={`relative w-7/12  min-h-[${slice.primary.heroimage.dimensions?.height}px] min-w-7/12 max-lg:w-0 max-lg:min-h-0`}>
        <Image fill={true} sizes="100vw" quality={100} src={String(slice.primary.heroimage.url)} className="object-fill" alt={''}  />
      </div>
    </section>
  );
};
