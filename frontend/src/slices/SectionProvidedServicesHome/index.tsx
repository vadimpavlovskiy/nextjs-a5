...
const SectionProvidedServicesHome = ({
  ...
  return (
    <section className="w-full p-10 justify-around bg-gray-100">
      ...
      <div className="flex flex-wrap justify-between md:gap-10 lg:gap-20">
          {slice.items.map((item:any) => {
            return (
              <div className="w-1/5 cursor-pointer group max-md:w-1/2 max-sm:w-full py-10 bg-white  drop-shadow-2xl  border-solid text-primary duration-500  border-2 flex gap-y-3 flex-col items-center justify-center rounded-xl hover:scale-95 hover:text-white hover:bg-primary">
                <PrismicNextImage className="w-1/4 group-hover:scale-110 group-hover:-translate-y-4 duration-500 bg-primary rounded-2xl p-1 hover:fill-white" loading={'lazy'} field={item.serviceimage} />
